...
add(bsm3, jd3_ssf_locallineartrend("lt", levelVariance = 0, 
                                   fixedLevelVariance = TRUE) )
add(bsm3, jd3_ssf_seasonal("s", 12, type="HarrisonStevens"))
add(bsm3, jd3_ssf_noise("n", 1, fixed=TRUE))
# create the equation (fix the variance to 1)
...
ll<-function(s, l){
  ...
  add(bsm2, jd3_ssf_seasonal("s", 12, type="Trigonometric", 
                             variance = 1, fixed = TRUE))
  add(bsm2, jd3_ssf_noise("n", 1, fixed=TRUE))
  # create the equation (fix the variance to 1)
  eq2<-jd3_ssf_equation("eq", 0, TRUE)
  add(eq2, "ll", sqrt(.000019), TRUE)
  add(eq2, "s", l, TRUE)
  add(eq2, "n", sqrt(0.000143), TRUE)
  add(bsm2, eq2)
  rslt2<-estimate(bsm2, log(s), marginal=TRUE)
...
ll2<-function(s, l){
  ...
  add(bsm2, jd3_ssf_noise("n", 0.000143, fixed=TRUE))
  # create the equation (fix the variance to 1)
  eq2<-jd3_ssf_equation("eq", 0, TRUE)
  add(eq2, "ll")
  add(eq2, "s")
